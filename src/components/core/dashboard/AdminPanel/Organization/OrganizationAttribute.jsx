...
import {
  ...
  deleteOrganisationAttributes,
  getOrganisationAttributes,
  updateOrganisationAttributes,
...
const OrganizationAttributes = ({ NextHandler }) => {
  ...
  const addAttributes = async () => {
    ...
    if (!normalizedAttributeKey) {
      ...
      setAttribute({
        ...attribute,
        error: "Attribute field must contain only letters",
      });
    } else {
      setAttribute({
        ...attribute,
      ...
      try {
        await dispatch(
          addOrganisationAttributes(AccessToken, {
            ...attribute,
            attributeKey: normalizedAttributeKey,
          })
        );
  ...
  const handleDelete = async (attributeId) => {
    await dispatch(deleteOrganisationAttributes(AccessToken, attributeId));
    getRes();
  };
  return (
    <div
      ...
      <div className="mt-4">
        {organizationAttribute.length > 0 ? (
          ...
          hover:scale-95 transition-all duration-200`}
                  ...
                  {editAttribute.error && (
                    <p className="text-red-500 mt-1">{editAttribute.error}</p>
                  )}
                </div>
              ) : (

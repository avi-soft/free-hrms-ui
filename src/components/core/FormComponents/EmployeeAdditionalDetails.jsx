...
import { FaArrowLeft, FaArrowRight, FaEdit, FaPlus, FaSave } from "react-icons/fa";
...
const EmployeeAdditionalDetails = () => {
  ...
  const AddEmergencyContactDetailsSubmit = (data) => {
    ...
    if (editedEmployeeId) {
      dispatch(
        addEmployeeEmergencyContactDetails(editedEmployeeId, formattedData?.contacts[0], AccessToken)
      );
    } else {
      console.log(formattedData?.contacts[0]);
      
      dispatch(
        addEmployeeEmergencyContactDetails(employeeId, formattedData?.contacts[0], AccessToken)
      );
    }
  };
  ...
  const AddSubmitAddress = (data) => {
    ...
    const employeeId = employees[0];
    if (editedEmployeeId) {
      dispatch(addEmployeeAddressDetails(editedEmployeeId, formattedData, AccessToken));
    } else {
      dispatch(addEmployeeAddressDetails(employeeId, formattedData, AccessToken));
    }
    setEditingAddressIndex(null);
  };
  ...
  const EditSubmitBank = (data) => {
    const formattedData = formatData(data);
    dispatch(EditEmployeeBankDetails(editedEmployeeId, formattedData, AccessToken));
  };
  ...
  const AddSubmitBank = (data) => {
    ...
    const employeeId = employees[0];
    if (editedEmployeeId) {
      dispatch(addEmployeeBankDetails(editedEmployeeId, formattedData, AccessToken));
    } else {
  ...
  const validateContact = (value) => {
    ...
    return true;
  };const validateRelationship = value => /^[A-Za-z\s]+$/.test(value) || 'Invalid relationship';
  ...
  const validateAccountNumber = value => {
    ...
    return regex.test(value) || 'Account Number must be between 9 and 18 digits';
  };
  ...
  return (
    <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="mt-2">
                  ...
                  {errorsEmergency?.contacts?.[index]?.contact && (
                    <p className="text-red-500 text-md mt-2">{errorsEmergency.contacts[index].contact.message}</p>
                  )}
                </div>
                <div className="mt-2">
                  <label
                  ...
                  {errorsEmergency?.contacts?.[index]?.relationship && (
                    <p className="text-red-500 text-md mt-2">{errorsEmergency.contacts[index].relationship.message}</p>
                  )}
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="mt-2">
                ...
                {errorsEmergency?.contacts?.[0]?.contact && (
                  <p className="text-red-500 text-md mt-2">{errorsEmergency.contacts[0].contact.message}</p>
                )}
              </div>
              <div className="mt-2">
                <label
                ...
                {errorsEmergency?.contacts?.[0]?.relationship && (
                  <p className="text-red-500 text-md mt-2">{errorsEmergency.contacts[0].relationship.message}</p>
                )}
              </div>
            </div>
            <div className="flex items-center gap-x-3 mt-2">
      ...
      <div className="address-section">
        <h2 className={`text-lg text-center ${darkMode ? 'text-white' : 'text-slate-600'} font-semibold mt-8`}>
        ...
        {isEditing && addressFields.length > 0 ? (
          addressFields.map((address, index) => (
            <form
              key={index}
              className="p-5"
              onSubmit={handleSubmitAddress((data) => EditSubmitAddress(data, index))}
            >
              <div className="grid grid-cols-2 gap-4">
                <div className="mt-2">
                  <label
                    htmlFor={`addresses[${index}].addressType`}
                    className={`block text-sm font-semibold ${darkMode ? 'text-white' : 'text-slate-900'}`}
                  >
                  ...
                  <select
                    ...
                    className={`border rounded px-3 py-2 mt-2 w-full ${darkMode ? 'border-slate-300 bg-gray-500 text-white' : 'border-slate-300 bg-white text-black'}`}
                  ...
                  {errorsAddress[`addresses[${index}].addressType`] && (
                    <p className="text-red-500 text-md mt-2">{errorsAddress[`addresses[${index}].addressType`]?.message}</p>
                  )}
                </div>
                <div className="mt-2">
                  <label
                  ...
                  <input
                    ...
                    {...registerAddress(`addresses[${index}].propertyNumber`, {
                  ...
                  {errorsAddress[`addresses[${index}].propertyNumber`] && (
                    <p className="text-red-500 text-md mt-2">{errorsAddress[`addresses[${index}].propertyNumber`]?.message}</p>
                  )}
                </div>
                <div className="mt-2">
                  <label
                  ...
                  {errorsAddress[`addresses[${index}].city`] && (
                    <p className="text-red-500 text-md mt-2">{errorsAddress[`addresses[${index}].city`]?.message}</p>
                  )}
                </div>
                <div className="mt-2">
                  <label
                  ...
                  <input
                    ...
                    {...registerAddress(`addresses[${index}].zipCode`, {
                      ...
                      pattern: {
                        value: /^[0-9]{6}(?:-[0-9]{4})?$/,
                        message: 'Zip Code must be in the format 123456 or 12345-6789',
                      },
                    })}
                    type="text"
                    className={`border rounded px-3 py-2 mt-2 w-full ${darkMode ? 'border-slate-300 bg-gray-500 text-white' : 'border-slate-300 bg-white text-black'}`}
                  ...
                  {errorsAddress[`addresses[${index}].zipCode`] && (
                    <p className="text-red-500 text-md mt-2">{errorsAddress[`addresses[${index}].zipCode`]?.message}</p>
                  )}
                </div>
                <div className="mt-2">
                  <label
                  ...
                  {errorsAddress[`addresses[${index}].state`] && (
                    <p className="text-red-500 text-md mt-2">{errorsAddress[`addresses[${index}].state`]?.message}</p>
                  )}
                </div>
                <div className="mt-2">
                  <label
                  ...
                  {errorsAddress[`addresses[${index}].country`] && (
                    <p className="text-red-500 text-md mt-2">{errorsAddress[`addresses[${index}].country`]?.message}</p>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-x-3 mt-4">
                <button
                  type="submit"
                  className={`text-center text-sm md:text-base font-medium rounded-md leading-6 hover:scale-95 transition-all duration-200 ${loading ? 'bg-slate-900 text-white' : 'bg-yellow-500 text-black'} py-1 px-5 flex items-center gap-3`}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="mt-2">
                ...
                {errorsAddress.addressType && (
                  <p className="text-red-500 text-md mt-2">{errorsAddress.addressType.message}</p>
                )}
              </div>
              <div className="mt-2">
                <label
                ...
                {errorsAddress.propertyNumber && (
                  <p className="text-red-500 text-md mt-2">{errorsAddress.propertyNumber.message}</p>
                )}
              </div>
              <div className="mt-2">
                <label
                ...
                />
                {errorsAddress.city && (
                  <p className="text-red-500 text-md mt-2">{errorsAddress.city.message}</p>
                )}
              </div>
              <div className="mt-2">
                <label
                ...
                {errorsAddress.zipCode && (
                  <p className="text-red-500 text-md mt-2">{errorsAddress.zipCode.message}</p>
                )}
              </div>
              <div className="mt-2">
                <label
                ...
                />
                {errorsAddress.state && (
                  <p className="text-red-500 text-md mt-2">{errorsAddress.state.message}</p>
                )}
              </div>
              <div className="mt-2">
                <label
                ...
                {errorsAddress.country && (
                  <p className="text-red-500 text-md mt-2">{errorsAddress.country.message}</p>
                )}
              </div>
            </div>
            <div className="flex items-center gap-x-3 mt-4">
      ...
      {isEditing && bankDetails ? (
        <div>
          ...
          <form className="p-5" onSubmit={handleSubmitBank(EditSubmitBank)}>
            <div className="grid grid-cols-2 gap-4">
              <div className="mt-2">
                <label htmlFor="bankName" className={`block text-sm font-semibold ${darkMode ? 'text-white' : 'text-slate-900'}`}>
                  Bank Name<sup className="text-red-900">*</sup>
                </label>
                <select
                  id="bankName"
                  {...registerBank('bankName', { validate: validateBankName })}
                  className={`border rounded px-3 py-2 mt-2 w-full ${darkMode ? 'border-slate-300 bg-gray-500 text-white' : 'border-slate-300 bg-white text-black'}`}
                >
              ...
              <div className="mt-2">
                <label htmlFor="accountNumber" className={`block text-sm font-semibold ${darkMode ? 'text-white' : 'text-slate-900'}`}>
                  Account Number<sup className="text-red-900">*</sup>
                </label>
                <input
                  id="accountNumber"
                  {...registerBank('accountNumber', { validate: validateAccountNumber })}
                  type="text"
                  className={`border rounded px-3 py-2 mt-2 w-full ${darkMode ? 'border-slate-300 bg-gray-500 text-white' : 'border-slate-300 bg-white text-black'}`}
              ...
              <div className="mt-2">
                <label htmlFor="ifsc" className={`block text-sm font-semibold ${darkMode ? 'text-white' : 'text-slate-900'}`}>
              ...
              <div className="mt-2">
                <label htmlFor="branch" className={`block text-sm font-semibold ${darkMode ? 'text-white' : 'text-slate-900'}`}>

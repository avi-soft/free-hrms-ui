...
const {
  ...
  EMPLOYEE_APPROVED_LEAVE_REQUESTS,
  EMPLOYEE_PENDING_LEAVE_REQUESTS,
  EMPLOYEE_LEAVE_LIST,
  EMPLOYEE_DECLINED_LEAVE_REQUESTS,
} = LeaveEndpoints;
...
export const AllLeaveRequests = (AccessToken) => {
  return async (dispatch) => {
    try {
      console.log(AccessToken);
      const response = await apiConnector("GET", ALL_LEAVE_REQUESTS, null, {
        Authorization: `Bearer ${AccessToken}`,
      });
      return response;
    } catch (err) {
